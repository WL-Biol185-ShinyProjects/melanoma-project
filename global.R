...
if ("fips_md" %in% colnames(md_availability)) {
  ...
  md_availability <- md_availability %>%
    ...
    left_join(county_lookup, by = c("county_clean", "state_clean"), relationship = "many-to-many") %>%
...
# Calculate US-wide breaks for MD availability bivariate map
# THIS MUST COME AFTER md_availability IS FULLY LOADED
us_wide_data_md <- melanoma_table %>%
  left_join(
    md_availability %>% select(fips_md, md_rate_per_100k),
    by = c("fips_melanoma" = "fips_md")
